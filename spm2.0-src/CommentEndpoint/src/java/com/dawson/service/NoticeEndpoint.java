...
//    教务员、教师，可以创建通知
    ...
    @JWTTokenNeeded({Role.manager, Role.teacher, Role.student})
    ...
    public Response deleteBelongTo(@PathParam("id") String id) {
        try {
            ...
            if (notice == null) {
                ...
                return Response.status(Status.FORBIDDEN).entity("评论不属于该用户不能删除").build();
            } else {
    ...
    public Response updateBelongTo(Notice entity) {
        try {
            if (entity.getId() == null) {
                return Response.status(Status.NOT_FOUND).entity("没有找到该评论").build();
            } else {
                Notice notice = this.noticeFacade.find(entity.getId());
                if (notice.getAuthor() == null || !notice.getAuthor().equals(entity.getAuthor())) {
                    return Response.status(Status.FORBIDDEN).entity("评论不属于该用户,不可以修改").build();
                } else {
